...
def possible_moves_list(id, posn):
    data=eval(database.getData(id))
    if data[2]=='w':
        color='w'
        p1_position_d=data[0]
    ...
    possible_posn=chess.find_moves(posn, p1_position_d, p2_position_d, color)
    if not(posn in data[0].keys()):
        return "{{possible_moves}}"
    code=open("templates/index.html", "r").read()
    code=code.replace("{{possible_moves}}", possible_posn)
    return render(id, "", code)
